    ...
    <div class="space-y-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-gray-500">Manage confirmed orders</p>
            </div>
            ...
            >
                <template #total_amount-data="{ row }">
                 ...
                 <template #payment_status-data="{ row }">
                    <UBadge :color="getPaymentColor(row.payment_status)" variant="soft">{{ row.payment_status }}</UBadge>
                </template>
        ...
        <FormSlideover
            v-model="isOpen"
            :title="editMode ? 'Edit Order ' + form.order_number : 'New Order'"
            :loading="saving"
            @submit="save"
        >
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-1 gap-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <UFormGroup label="Payment Status">
                             <USelect v-model="form.payment_status" :options="['unpaid', 'partial', 'paid']" />
                        </UFormGroup>
                    </div>
                </div>
...
const fetchData = async () => {
    loading.value = true
    try {
        const res: any = await $api.get('/sales/orders', {
            params: {
...
const save = async () => {
    ...
    try {
        if (editMode.value) {
            await $api.put(`/sales/orders/${form.value.id}`, form.value)
