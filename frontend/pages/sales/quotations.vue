    ...
    <div class="space-y-4">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-gray-500">Manage customer quotations</p>
            </div>
            <UButton icon="i-heroicons-plus" @click="openCreate">New Quotation</UButton>
        </div>
        ...
        <FormSlideover
            v-model="isOpen"
            :title="editMode ? 'Edit Quotation ' + form.quotation_number : 'New Quotation'"
            :loading="saving"
            @submit="save"
        >
            <div class="space-y-6">
...
// Data Handling (Manual Fetch)
const fetchData = async () => {
    loading.value = true
    try {
        const res: any = await $api.get('/sales/quotations', {
            params: {
...
const openEdit = async (row: any) => {
    try {
        loading.value = true
        const res = await $api.get(`/sales/quotations/${row.id}`)
        if (res.data.success) {
...
const save = async () => {
    ...
    try {
        if (editMode.value) {
            await $api.put(`/sales/quotations/${form.value.id}`, form.value)
             toast.add({ title: 'Updated', color: 'green' })
        } else {
            await $api.post('/sales/quotations', form.value)
...
const deleteItem = async (row: any) => {
    ...
    try {
       // Assuming generic delete endpoint exists
       // await $api.delete(`/sales/quotations/${row.id}`)
