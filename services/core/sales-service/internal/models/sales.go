...
type Quotation struct {
	...
	ValidUntil      *Date           `gorm:"type:date" json:"valid_until"`
...
type SalesOrder struct {
	...
	CustomerID     *string          `gorm:"type:uuid" json:"customer_id"`
	Date           Date             `gorm:"type:date;default:CURRENT_DATE" json:"date"`
	DeliveryDate   *Date            `gorm:"type:date" json:"delivery_date"`
...
type Invoice struct {
	...
	CustomerID       *string       `gorm:"type:uuid" json:"customer_id"`
	Date             Date          `gorm:"type:date;default:CURRENT_DATE" json:"date"`
	DueDate          *Date         `gorm:"type:date" json:"due_date"`
	...
	LoyaltyCoinsUsed float64       `gorm:"type:decimal(15,2);default:0" json:"loyalty_coins_used"`
...
func (item *QuotationItem) BeforeCreate(tx *gorm.DB) (err error) {
	if item.ID == "" {
...
func (item *SalesOrderItem) BeforeCreate(tx *gorm.DB) (err error) {
	if item.ID == "" {
		item.ID = uuid.New().String()
	}
	return
